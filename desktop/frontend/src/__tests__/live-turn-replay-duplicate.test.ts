...
//
// Regression (test/live-dup line): leaving a session that is mid-turn and
// coming back renders the turn's rows a second time.
//
...
// reducer applies the replayed events on top of a transcript that ALREADY
// holds everything the turn had produced. Nothing clears the turn being
// re-projected, so the replay would append a parallel copy of it unless the
// active turn's segment allocation is rewound first (replay_turn_reset) and
// ensureAssistant reuses the rows that already hold the derived ids.

import type { AppBindings } from "../lib/bridge";
import type { TurnEventReplayView } from "../lib/types";

// The active turn's events, starting at the turn's first event (turnStartSeq).
...
// into a row, a tool call is running, and the answer segment is streaming.
const turn2Before = [
...
// runtime.rebuilt makes the tab momentarily inactive, and the backend_status
// that follows clears turnActive/running. A gap replay still re-projects the
// turn in that state, so the reset must not depend on those flags being set.
{
  let idleState = {
    ...initialState,
    items: [
