...
func TestForkForTabCreatesNewSessionTab(t *testing.T) {
	fx := newSchemaTwoTabFixture(t)
	meta, err := fx.app.ForkForTab("test", 1)
	if err != nil {
		t.Fatalf("ForkForTab: %v", err)
	}
	if meta.ID == "" || meta.ID == "test" || !meta.Active || meta.SessionPath == fx.path {
		t.Fatalf("fork meta = id %q active %v path %q, want a brand-new active tab with its own session file", meta.ID, meta.Active, meta.SessionPath)
	}
	if len(fx.app.tabs) != 2 || fx.ctrl.SessionPath() != fx.path {
		t.Fatalf("tabs = %d source path %q, want source tab preserved plus one fork tab", len(fx.app.tabs), fx.ctrl.SessionPath())
	}
	if got := len(fx.ctrl.History()); got != 5 {
		t.Fatalf("source history after fork = %d, want the full 5 messages unchanged", got)
	}
	if got := transcriptFilesIn(t, filepath.Dir(fx.path)); got != 2 {
		t.Fatalf("transcript files = %d, want the source log plus the new fork file", got)
	}
	heads, err := agent.ListSessionHeads(fx.path)
	if err != nil || len(heads) != 1 {
		t.Fatalf("source heads = %+v err=%v, want the main head untouched (fork no longer switches heads)", heads, err)
	}
}
...
func TestChooseRecoveryBranchSwitchesOpenTabHeadInPlace(t *testing.T) {
	fx := newSchemaTwoTabFixture(t)
	// Create the fork head through the controller directly: the ForkForTab
	// button now creates a separate session file, so it can no longer seed an
	// in-log head for the recovery-branch scenario.
	if _, err := fx.ctrl.ForkNamed(1, ""); err != nil {
		t.Fatal(err)
	}
	heads, _ := agent.ListSessionHeads(fx.path)
	fork := heads[1].ID
	before := fx.app.tabs["test"].SessionGeneration
	...
	if gen := fx.app.tabs["test"].SessionGeneration; gen != before+1 {
		t.Fatalf("session generation = %d, want a bump per head switch (was %d)", gen, before)
	}
